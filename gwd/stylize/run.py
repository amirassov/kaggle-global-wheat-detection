...
parser.add_argument(
    "--content-dir", default="/data/SPIKE_Dataset/images", help="Directory path to a batch of content images"
)
parser.add_argument("--style-dir", default="/data/test", help="Directory path to a batch of style images")
parser.add_argument(
    "--output-dir", default="/data/SPIKE_Dataset/stylized_images_v4", help="Directory to save the output images"
)
parser.add_argument(
    "--num-styles",
    type=int,
    default=1,
    help="Number of styles to \
        ...
        for content_path in content_paths:
            try:
                ...
                for style_path in random.sample(styles, args.num_styles):
                    ...
                    # out_filename = (
                    #     content_name + "-stylized-" + style_name + content_path.suffix
                    # )
                    out_filename = content_name + content_path.suffix
                    output_name = out_dir.joinpath(out_filename)

                    # save_image(
                    #     torch.cat([style.cpu(), content.cpu(), output]), output_name, padding=0
                    # )
                    save_image(output, output_name, padding=0)  # default image padding is 2.
                    style_img.close()
                content_img.close()
            except OSError as e:
                print("Skipping stylization of %s due to an error" % (content_path), e)
