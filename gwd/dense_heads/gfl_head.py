...
class ModifiedGFLHead(AnchorHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.reg_max = reg_max
        super(ModifiedGFLHead, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def loss_single(
        self, anchors, cls_score, bbox_pred, labels, label_weights, bbox_targets, stride, num_total_samples
    ):
        ...
        if len(pos_inds) > 0:
            ...
            loss_dfl = self.loss_dfl(
                pred_corners, target_corners, weight=weight_targets[:, None].expand(-1, 4).reshape(-1), avg_factor=4.0
            )
        else:
    ...
    @force_fp32(apply_to=("cls_scores", "bbox_preds"))
    def get_bboxes(self, cls_scores, bbox_preds, img_metas, cfg=None, rescale=False, nms=True):
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(
                cls_score_list, bbox_pred_list, mlvl_anchors, img_shape, scale_factor, cfg, rescale, nms
            )
            result_list.append(proposals)
        return result_list

    def _get_bboxes_single(
        self, cls_scores, bbox_preds, mlvl_anchors, img_shape, scale_factor, cfg, rescale=False, nms=True
    ):
        ...
        if nms:
            ...
            return (
                mlvl_bboxes,
                mlvl_scores,
                torch.ones(len(mlvl_scores), dtype=mlvl_scores.dtype, device=mlvl_scores.device),
            )
...
@HEADS.register_module()
class GFLSEPCHead(ModifiedGFLHead):
    def forward_single(self, x, scale):
