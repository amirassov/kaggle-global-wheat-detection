...
class RecursiveFeaturePyramid(HybridTaskCascade):
    def __init__(self, backbone, rfp_steps=2, rfp_sharing=False, stage_with_rfp=(False, True, True, True), **kwargs):
        ...
        self.rfp_weight = torch.nn.Conv2d(neck_out_channels, 1, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def extract_feat(self, img):
        ...
        for rfp_idx in range(self.rfp_steps - 1):
            rfp_feats = tuple(
                self.rfp_aspp(x[i]) if self.stage_with_rfp[i] else x[i] for i in range(len(self.stage_with_rfp))
            )
            if self.rfp_sharing:
