...
from .sepc_dconv import SEPCConv
...
class SEPC(nn.Module):
    def __init__(
        ...
        self.lconv = SEPCConv(256, 256, kernel_size=3, padding=lcconv_padding, dilation=1, part_deform=lcconv_deform)
        self.cconv = SEPCConv(256, 256, kernel_size=3, padding=lcconv_padding, dilation=1, part_deform=lcconv_deform)
        self.relu = nn.ReLU()
        if self.ibn:
