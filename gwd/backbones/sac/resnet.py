...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.rfp = rfp
        if self.rfp:
            self.rfp_conv = torch.nn.Conv2d(self.rfp, planes * self.expansion, kernel_size=1, stride=1, bias=True)
...
def make_res_layer(
    ...
    if stride != 1 or inplanes != planes * block.expansion:
        downsample = nn.Sequential(
            build_conv_layer(conv_cfg, inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False),
...
@BACKBONES.register_module(name="DetectoRS_ResNetV1")
class DetectoRS_ResNet(nn.Module):
    """ResNet backbone.
    ...
    def __init__(
        ...
        super(DetectoRS_ResNet, self).__init__()
    ...
    def train(self, mode=True):
        super(DetectoRS_ResNet, self).train(mode)
        self._freeze_stages()
